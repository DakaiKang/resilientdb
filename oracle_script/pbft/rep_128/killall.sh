...
do
	echo ${ip}
	ssh -i ~/.ssh/dakai_dev.pem -n -o BatchMode=yes -o StrictHostKeyChecking=no ubuntu@${ip} " killall -9 kv_server_performance; killall -9 benchmark_server;" &
	((count++))
done
...
scp -i ~/.ssh/dakai_dev.pem /home/ubuntu/resilientdb/oracle_script/pbft/rep_128/server.config ubuntu@${ip}:/home/ubuntu &
scp -i ~/.ssh/dakai_dev.pem /home/ubuntu/resilientdb/oracle_script/pbft/rep_128/cert/node_${idx}.key.pri ubuntu@${ip}:/home/ubuntu/pbft_cert/ &
scp -i ~/.ssh/dakai_dev.pem /home/ubuntu/resilientdb/oracle_script/pbft/rep_128/cert/cert_${idx}.cert ubuntu@${ip}:/home/ubuntu/pbft_cert/ &
	((count++))
	((count++))
	((count++))
	((count++))
